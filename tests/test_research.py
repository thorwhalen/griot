...
def wiki_http(*titles: str, extract: str = "Lead paragraph.\n\n== History ==\nBuilt in 1846.") -> FakeHttp:
    def api(params):
        if params.get("list") == "search":
            return {"query": {"search": [{"title": t} for t in titles]}}
        return {"query": {"pages": {"1": {"extract": extract}}}}

    return FakeHttp(json_routes={WIKI_API: api, WIKI_SUMMARY: {"description": "a church", "extract": "Lead paragraph."}})


def test_split_topic():
    assert split_topic("The Sound of Silence by Simon & Garfunkel") == ("The Sound of Silence", "Simon & Garfunkel")
...
def test_best_hit_prefers_the_exact_title_over_the_album_ranked_first():
    hits = [{"title": "Sounds of Silence"}, {"title": "The Sound of Silence"}, {"title": "Simon & Garfunkel"}]
    assert best_hit(hits, "The Sound of Silence", artist="Simon & Garfunkel")["title"] == "The Sound of Silence"
...
def test_wikipedia_resolves_and_cites_every_fact():
    ...
    assert all(f.source_url == "https://en.wikipedia.org/wiki/Trinity_Church_(Manhattan)" for f in d.facts)
    assert d.sources[0].kind == "wikipedia" and not d.missing


def test_wikipedia_refuses_the_nearest_match():
    d = research("Actually Sweet by Taylor Swift", researchers=("wikipedia",), http=wiki_http("Actually Romantic", "Taylor Swift"))
...
def test_a_failing_researcher_is_recorded_not_raised():
    ...
    assert d.missing[0].startswith("genius: failed") and d.missing[1].startswith("wikipedia: failed")
    assert "NOT FOUND" in d.as_brief()


def test_lrclib_times_and_carries_plain_lyrics_from_the_topic_alone():
    rec = {"id": 595765, "trackName": "The Sound of Silence", "artistName": "Simon & Garfunkel", "plainLyrics": "Hello darkness, my old friend\nI've come to talk with you again", "syncedLyrics": "[00:00.50] Hello darkness, my old friend\n[00:04.20] I've come to talk with you again\n[00:08.00] "}
    http = FakeHttp(json_routes={"https://lrclib.net/api/get": rec})
    d = research("The Sound of Silence by Simon & Garfunkel", researchers=("lrclib",), http=http)
    assert d.subject == "The Sound of Silence" and d.artist == "Simon & Garfunkel"
    assert d.lyrics.startswith("Hello darkness")
    assert [(t.index, t.start_s, t.end_s) for t in d.timed_lines] == [(0, 0.5, 4.2), (1, 4.2, 8.0)]
...
def test_genius_uses_the_official_api_with_a_token(monkeypatch):
    ...
    stub = {"id": 79538, "title": "The Sound of Silence", "full_title": "The Sound of Silence by Simon & Garfunkel", "url": "https://genius.com/x-lyrics", "primary_artist": {"name": "Simon & Garfunkel"}}
    http = FakeHttp(
        json_routes={
            "https://api.genius.com/search": {"response": {"hits": [{"result": stub}]}},
            "https://api.genius.com/songs/79538": {"response": {"song": {**stub, "release_date_for_display": "1964"}}},
            "https://api.genius.com/referents": {"response": {"referents": [{"fragment": "Hello darkness", "url": "u", "annotations": [{"body": {"plain": "bathroom, lights off"}, "verified": True, "votes_total": 5}]}]}},
        },
        text_routes={"https://genius.com/x-lyrics": '<div data-lyrics-container="true">Hello darkness<br/>my old friend</div>'},
    )
    d = research("The Sound of Silence by Simon & Garfunkel", researchers=("genius",), http=http)
    assert d.subject == "The Sound of Silence" and d.artist == "Simon & Garfunkel"
    ...
    assert len(api_calls) == 3 and all(u.startswith("https://api.genius.com") for u in api_calls)
    assert not d.missing


def test_genius_lyrics_page_refusal_is_soft(monkeypatch):
    monkeypatch.setenv("GENIUS_ACCESS_TOKEN", "t0k")
    stub = {"id": 1, "title": "Burn", "url": "https://genius.com/burn-lyrics", "primary_artist": {"name": "Cast"}}
    http = FakeHttp(json_routes={"https://api.genius.com/search": {"response": {"hits": [{"result": stub}]}}, "https://api.genius.com/songs/1": {"response": {"song": stub}}, "https://api.genius.com/referents": {"response": {"referents": []}}})
...
def test_merge_keeps_first_scalars_and_concatenates_sequences():
    a = griot.Dossier(topic="t", subject="A", facts=(griot.Fact("1", "u1"),), sources=(griot.Source("u1"),))
    b = griot.Dossier(topic="t", subject="B", artist="X", facts=(griot.Fact("2", "u2"),), sources=(griot.Source("u1"), griot.Source("u2")))
    m = griot.merge("t", [a, b])
    assert m.subject == "A" and m.artist == "X"
    assert [f.text for f in m.facts] == ["1", "2"] and [s.url for s in m.sources] == ["u1", "u2"]
